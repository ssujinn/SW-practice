...
double genRandN_Secant(double U, int si) {
	double L = 0.0, R = 1.0;
	double x0 , x1 = (L + R) / 2;// these variables are for initial build without coding
	...
	for (iter = 0; iter <= maxIter; iter++) {
		...
		tmp = x1;
		x1 = x1 - (interpolationF(x1, U))*((x1 - x0) / ((interpolationF(x1, U)) - (interpolationF(x0, U))));
		x0 = tmp;
...
double genRandN_Improved(double U, int si) {
	double L = 0.0, R = 1.0;
	double x1 = (L + R) / 2, x0 = 0;		// these variables are for initial build without coding
										// you may change the names or delete them if needed
	...
	for (iter = 0; iter <= maxIter; iter++) {
		...
		x0 = x1;
		nrvar = x0 - (interpolationF(x0, U) / interpolationFD(x0));
