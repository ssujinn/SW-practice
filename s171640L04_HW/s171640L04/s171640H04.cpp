...
void finalOutput(int problemID, double parameters[], double ans, 
	...
	double y_value = parameters[0] * ans + parameters[1];
	switch (problemID) {
	case PROBLEM1:
	...
	case PROBLEM2:
		...
		break;
	case PROBLEM3:
		printf("ans: x= %.18e y = %.18e |f(x)|= %.10e iteration= %2d\n", ans, y_value, absfval, iteration);
		break;
	case PROBLEM4:
		printf("ans: x= %.18e |f(x)|= %.10e iteration= %2d\n", ans, absfval, iteration);
		break;
	case PROBLEM5:
		printf("ans: r= %.4lf |f(x)|= %.10e iteration= %2d\n", ans, absfval, iteration);
		break;
...
if (showSteps == OUTPUT_STEPS) {
	if (chkConvergence == CONVERGENCE_CHECK)
		printf(" n              xn1           |f(xn1)|     e1       c=e1/(e0^1.62)\n");
	else
		printf(" n              xn1           |f(xn1)|\n");
}
for (iter = 0; iter <= maxIter; iter++) {
	if (showSteps == OUTPUT_STEPS) {
		...
		if (iter == 0 || chkConvergence != CONVERGENCE_CHECK) {
			...
			e0 = fabs(x0 - root);
			e1 = fabs(x1 - root);
			c = e1 / pow(e0, 1.62);  // 이 값이 어느 정도 상수 값인지 조사하자
			printf("%2d  %.18e  %.2e %.6e %.6e\n", iter, x1, fabs(_F(x1, p[0], p[1], p[2])), e1, c);
		}
	}
	if (fabs(_F(x1, p[0], p[1], p[2])) < delta)
	...
	// 다음 세 줄이 Secant method core 입니다
	tmp = x1;
	x1 = x1 - (_F(x1, p[0], p[1], p[2]))*((x1 - x0)/(_F(x1, p[0], p[1], p[2]) - _F(x0, p[0], p[1], p[2])));
	x0 = tmp;
}
// 최종 출력을 저장합니다
*iteration = iter;	// 총 반복 회수
	...
	for (iter = 0; iter <= maxIter; iter++) {
		if (showSteps == OUTPUT_STEPS) {
			...
			if (iter == 0 || chkConvergence != CONVERGENCE_CHECK) {
				...
				e0 = fabs(x0 - root);
				e1 = fabs(x1 - root);
				c = e1 / e0; // 이 값이 어느 정도 상수 값인지 조사하자
				printf("%2d  %.18e  %.2e %.6e %.6e\n", iter, x1, fabs(_F(x1, p[0], p[1], p[2])), e1, c);
			}
		}
		if (fabs(_F(x1, p[0], p[1], p[2])) < delta)
		...
		// 다음 코드가 Bisection method core 입니다
		x0 = x1;
